	...
	if (_actionsSheet) {
		...
		if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
			...
			self.actionsSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self
												   cancelButtonTitle:NSLocalizedString(@"Cancel", @"取消") destructiveButtonTitle:nil
												   otherButtonTitles:NSLocalizedString(@"Save", @"保存"), NSLocalizedString(@"Copy", @"拷贝"), nil];
...
#ifdef _LoadingToastForAction
	[self.view toastWithLoading:message];
#endif
...
- (void)showProgressHUDCompleteMessage:(NSString *)message {
	[self.view dismissToast];
	if (message) {
		[self.view toastWithSuccess:message];
	}
	self.navigationController.navigationBar.userInteractionEnabled = YES;
}
...
- (void)savePhoto {
	id <MWPhoto> photo = [self photoAtIndex:_currentPageIndex];
	if ([photo underlyingImage]) {
		[self showProgressHUDWithMessage:[NSString stringWithFormat:@"%@\u2026" , NSLocalizedString(@"Saving", @"正在保存")]];
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
	[self showProgressHUDCompleteMessage: error ? NSLocalizedString(@"Failed", @"失败") : NSLocalizedString(@"Saved", @"已保存到相册")];
	[self hideControlsAfterDelay]; // Continue as normal...
}
...
- (void)copyPhoto {
	id <MWPhoto> photo = [self photoAtIndex:_currentPageIndex];
	if ([photo underlyingImage]) {
		[self showProgressHUDWithMessage:[NSString stringWithFormat:@"%@\u2026" , NSLocalizedString(@"Copying", @"正在拷贝")]];
...
- (void)actuallyCopyPhoto:(id<MWPhoto>)photo {
	if ([photo underlyingImage]) {
		...
		[self showProgressHUDCompleteMessage:NSLocalizedString(@"Copied", @"已拷贝")];
		[self hideControlsAfterDelay]; // Continue as normal...
	}
}
