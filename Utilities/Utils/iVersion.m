...
- (void)downloadedVersionsData
{
    ...
    if (self.checkingForNewVersion)
    {
        ...
        if (!self.remoteVersionsDict)
        {
            ...
            {
                _Log(@"iVersion update check failed because: %@", [self.downloadError localizedDescription]);
            }
            else
            {
                _Log(@"iVersion update check failed because an unknown error occured");
            }
        ...
        if (details)
        {
            ...
            if (showDetails)
            {
                if ([self.delegate respondsToSelector:@selector(iVersionShouldDisplayNewVersion:details:)])
                {
                    ...
                    {
                        _Log(@"iVersion did not display the new version because the iVersionShouldDisplayNewVersion:details: delegate method returned NO");
                    }
                }
            }
            else if (self.verboseLogging)
            {
                _Log(@"iVersion did not display the new version because it was marked as ignored");
            }
            
            //show details
            if (showDetails && !self.visibleRemoteAlert)
            {
                NSString *title = iVersionUpdateAvailableTitle;
                if (!self.groupNotesByVersion)
                {
                ...
                self.visibleRemoteAlert = [self alertViewWithTitle:title
                                                           details:details
                                                     defaultButton:iVersionDownloadButtonLabel
                                                      cancelButton:iVersionIgnoreButtonLabel];
                
                if ([iVersionRemindButtonLabel length])
                {
                    [self.visibleRemoteAlert addButtonWithTitle:iVersionRemindButtonLabel];
                }
...
- (BOOL)shouldCheckForNewVersion
{
    ...
    if (!self.previewMode)
    {
        ...
        if (self.lastReminded != nil)
        {
            ...
            if ([[NSDate date] timeIntervalSinceDate:self.lastReminded] < self.remindPeriod * SECONDS_IN_A_DAY)
            {
                ...
                {
                    _Log(@"iVersion did not check for a new version because the user last asked to be reminded less than %g days ago", self.remindPeriod);
                }
                return NO;
            }
        }
        ...
        else if (self.lastChecked != nil && [[NSDate date] timeIntervalSinceDate:self.lastChecked] < self.checkPeriod * SECONDS_IN_A_DAY)
        {
            if (self.verboseLogging)
            {
                _Log(@"iVersion did not check for a new version because the last check was less than %g days ago", self.checkPeriod);
            }
            return NO;
        }
    }
    ...
    {
        _Log(@"iVersion debug mode is enabled - make sure you disable this for release");
    }
    ...
    if ([self.delegate respondsToSelector:@selector(iVersionShouldCheckForNewVersion)])
    {
        ...
        {
            _Log(@"iVersion did not check for a new version because the iVersionShouldCheckForNewVersion delegate method returned NO");
        }
        return shouldCheck;
    }
...
- (void)checkForNewVersionInBackground
{
    @synchronized (self)
    {
        @autoreleasepool
        {
            ...
            if (self.verboseLogging)
            {
                _Log(@"iVersion is checking %@ for a new app version...", iTunesServiceURL);
            }
            ...
            if (data && statusCode == 200)
            {
                ...
                if (!error)
                {
                    ...
                    if (bundleID)
                    {
                        if ([bundleID isEqualToString:self.applicationBundleID])
                        {
                            ...
                            if (!self.appStoreID)
                            {
                                ...
                                if (self.verboseLogging)
                                {
                                    _Log(@"iVersion found the app on iTunes. The App Store ID is %@", appStoreIDString);
                                }
                            }
                            ...
                            if (self.verboseLogging)
                            {
                                ...
                                {
                                    _Log(@"iVersion found a new version (%@) of the app on iTunes. Current version is %@", latestVersion, self.applicationVersion);
                                }
                                else
                                {
                                    _Log(@"iVersion did not find a new version of the app on iTunes. Current version is %@, latest version is %@", self.applicationVersion, latestVersion);
                                }
                            }
                        }
                        else
                        {
                            if (self.verboseLogging)
                            {
                                _Log(@"iVersion found that the application bundle ID (%@) does not match the bundle ID of the app found on iTunes (%@) with the specified App Store ID (%@)", self.applicationBundleID, bundleID, @(self.appStoreID));
                            }
                    ...
                    else if (self.appStoreID || !self.remoteVersionsPlistURL)
                    {
                        if (self.verboseLogging)
                        {
                            _Log(@"iVersion could not find this application on iTunes. If your app is not intended for App Store release then you must specify a remoteVersionsPlistURL. If this is the first release of your application then it's not a problem that it cannot be found on the store yet");
                        }
                    ...
                    {
                        _Log(@"iVersion could not find your app on iTunes. If your app is not yet on the store or is not intended for App Store release then don't worry about this");
                    }
                    ...
                    if (((self.appStoreID && newerVersionAvailable) || !self.appStoreID || self.previewMode) && self.remoteVersionsPlistURL)
                    {
                        if (self.verboseLogging)
                        {
                            _Log(@"iVersion will check %@ for %@", self.remoteVersionsPlistURL, self.appStoreID? @"release notes": @"a new app version");
                        }
                        ...
                        {
                            _Log(@"iVersion was unable to download the user-specified release notes");
                        }
                    }
                }
...
- (void)checkIfNewVersion
{
    ...
    if (self.lastVersion != nil || self.showOnFirstLaunch || self.previewMode)
    {
        if ([self.applicationVersion compareVersion:self.lastVersion] == NSOrderedDescending || self.previewMode)
        {
            ...
            {
                self.visibleLocalAlert = [self alertViewWithTitle:iVersionInThisVersionTitle
                                                          details:self.versionDetails
                                                    defaultButton:iVersionOKButtonLabel
                                                     cancelButton:nil];
...
- (BOOL)openAppPageInAppStore
{
    if (!_updateURL && !self.appStoreID)
    {
        if (self.verboseLogging)
        {
            _Log(@"iVersion was unable to open the App Store because the app store ID is not set.");
        }
        return NO;
    }
    ...
    if (!_updateURL && [SKStoreProductViewController class])
    {
        if (self.verboseLogging)
        {
            _Log(@"iVersion will attempt to open the StoreKit in-app product page using the following app store ID: %@", @(self.appStoreID));
        }
        ...
        if (!rootViewController)
        {
            if (self.verboseLogging)
            {
                _Log(@"iVersion couldn't find a root view controller from which to display StoreKit product page");
            }
        }
        else
        {
    ...
    if (self.verboseLogging)
    {
        _Log(@"iVersion will open the App Store using the following URL: %@", self.updateURL);
    }
...
- (void)applicationLaunched
{
    ...
    {
        _Log(@"iVersion will not check for updates because the checkAtLaunch option is disabled");
    }
}
